...
describe("checks", function() {
  describe("#expose_container_port_only(args)", function() {
    it("validates expose command for no host port", function() {
      ...
      expect(checks.expose_container_port_only("8000 8001:8000"))
          .to.have.length(1);
    });
  });
  ...
  describe("#valid_maintainer(args)", function() {
    it("validates maintainer command has exactly one maintainer and outputs MAINTAINER is deprecated in 1.13",
       function() {
         expect(checks.valid_maintainer("user <test@gmail.com>"))
             .to.have.length(1)
             .and.to.eql([ 'deprecated_in_1.13' ]);
         expect(checks.valid_maintainer("user without email"))
             .to.have.length(2);
         expect(checks.valid_maintainer(
                    "user <test@gmail.com> user2 <test@gmail.com>"))
             .to.have.length(2);
       });
  });

  describe("#param_count_min(args, min_count)", function() {
    it("validates that a argument line, split on whitespace, contains at least min_count elements",
       function() {
  ...
  describe("#is_dir_in_context(dir)", function() {
    it("validates that a directory is relative (to the docker context)",
       function() {
  ...
  describe(
      "#is_wrapped_in_quotes(str)",
      function() {
        it("returns a bool indicating if there are matching quotes on each side of a string",
           function() {
