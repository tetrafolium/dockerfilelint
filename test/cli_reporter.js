  ...
  describe('#buildReport()', () => {
    ...
    it('returns green report when one file added with no issues', () => {
      ...
      expect(report.toString().split('\n')).to.deep.equal([
        '', 'File:   ' + file, 'Issues: ' + chalk.green('None found') + ' 👍', ''
      ]);
    });
    ...
    it('returns green report when multiple files added with no issues', () => {
      ...
      expect(report.totalIssues).to.equal(0);
      expect(report.toString().split('\n')).to.deep.equal([
        '', 'File:   ' + file1, 'Issues: ' + chalk.green('None found') + ' 👍',
        '', 'File:   ' + file2, 'Issues: ' + chalk.green('None found') + ' 👍',
        ''
      ]);
    });
    ...
    it('returns pretty report when file added with issues', () => {
      ...
      expect(report.toString().split('\n')).to.deep.equal([
        ...
        '    ' + chalk.cyan('1') + '  ' + chalk.cyan.inverse('Clarity') +
            '       ' + chalk.cyan('Base Image Missing') + '    ' +
            chalk.gray('Base images should specify a tag to use.'),
        '                     ' + chalk.cyan('Tag'),
        '',
        'Line 6: ' + chalk.magenta('FROM ubuntu:latest'),
        'Issue  Category      Title                 Description',
        '    ' + chalk.yellow('2') + '  ' +
            chalk.yellow.inverse('Possible Bug') + '  ' +
            chalk.yellow('First Command Must') + '    ' +
            chalk.gray(
                'The first instruction in a Dockerfile must specify the base image'),
        '                     ' + chalk.yellow('Be FROM') + '               ' +
            chalk.gray(
                'using a FROM command.  Additionally, FROM cannot appear later in a'),
        '                                           ' +
            chalk.gray('Dockerfile.'),
        '    ' + chalk.cyan('3') + '  ' + chalk.cyan.inverse('Clarity') +
            '       ' + chalk.cyan('Base Image Latest') + '     ' +
            chalk.gray('Base images should not use the latest tag.'),
        '                     ' + chalk.cyan('Tag'),
        '',
        'Line 25: ' + chalk.magenta('EXPOSE 80:80'),
        'Issue  Category      Title                 Description',
        '    ' + chalk.red('4') + '  ' + chalk.red.inverse('Deprecation') +
            '   ' + chalk.red('Expose Only') + '           ' +
            chalk.gray('Using `EXPOSE` to specify a host port is not allowed.'),
    ...
    it('uses Clarity style for unknown category', () => {
      ...
      expect(report.toString().split('\n')).to.deep.equal([
        ...
        'Issue  Category      Title                 Description',
        '    ' + chalk.cyan('1') + '  ' + chalk.cyan.inverse('Hello') +
            '         ' + chalk.cyan('Hello World!') + '          ' +
            chalk.gray('This is a test.'),
        ''
      ]);
    });
  });
