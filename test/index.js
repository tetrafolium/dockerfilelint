...
describe("index", function() {
  ...
  describe("#multistage", function() {
    it("validates the multistage Dockerfile has no issues", function() {
      expect(dockerfilelint.run(
                 './test/examples',
                 fs.readFileSync('./test/examples/Dockerfile.multistage',
                                 'UTF-8')))
          .to.be.empty;
    });
    ...
    it("validates the multistagenamed Dockerfile has no issues", function() {
      expect(dockerfilelint.run(
                 './test/examples',
                 fs.readFileSync('./test/examples/Dockerfile.multistagenamed',
                                 'UTF-8')))
          .to.be.empty;
    });
  });
  ...
  describe("#arg-before-from", function() {
    it("validates the Dockerfile has no issues", function() {
      const lintResult = dockerfilelint.run(
          './test/examples',
          fs.readFileSync('./test/examples/Dockerfile.arg-before-from',
                          'UTF-8'));
  ...
  describe("#from-with-as", function() {
    it("validates the Dockerfile detects latest when using as", function() {
      var expected =
          [ {title : 'Base Image Latest Tag', line : 1, rule : 'latest_tag'} ];
      const lintResult = dockerfilelint.run(
          './test/examples',
          fs.readFileSync('./test/examples/Dockerfile.from-as', 'UTF-8'));
  ...
  describe("#from-with-port", function() {
    it("validates the Dockerfile detects latest when it has a port",
       function() {
         var expected = [
           {title : 'Base Image Latest Tag', line : 1, rule : 'latest_tag'}
         ];
         const lintResult = dockerfilelint.run(
             './test/examples',
             fs.readFileSync('./test/examples/Dockerfile.from-with-port',
                             'UTF-8'));
         _.forEach(lintResult, function(r) {
           delete r['description'];
           delete r['category'];
  ...
  describe("#from-with-port-no-latest", function() {
    it("validates the Dockerfile detects not latest when it has a port",
       function() {
         const lintResult = dockerfilelint.run(
             './test/examples',
             fs.readFileSync(
                 './test/examples/Dockerfile.from-with-port-no-latest',
                 'UTF-8'));
  ...
  describe("#shell", function() {
    it("validates the shell command is accepted when entered correctly",
       function() {
         expect(dockerfilelint.run(
                    './test/examples',
                    fs.readFileSync('./test/examples/Dockerfile.shell.pass',
                                    'UTF-8')))
             .to.be.empty;
       });

    it("validates the shell command detects invalid shell commands",
       function() {
         var expected = [ {
  ...
  describe("#misc", function() {
    it("validates the misc Dockerfile have the exact right issues reported",
       function() {
         var expected = [
           {
