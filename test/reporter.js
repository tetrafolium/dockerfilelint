...
describe('reporter', () => {
  ...
  describe('#addFile(file, fileContent, items)', () => {
    ...
    it('adds a good report when no items given', () => {
      ...
      let reporter =
          new Reporter().addFile(file, fs.readFileSync(file, 'UTF-8'), []);
    ...
    it('modifies existing report when same file added twice', () => {
      ...
      expect(fileReport.itemsByLine).to.deep.equal({'5' : [ item ]});
    });
    ...
    it('groups multiple items by line number', () => {
      ...
      expect(fileReport.itemsByLine)
          .to.deep.equal({'5' : [ items[0] ], '6' : items.splice(1)});
    });
    ...
    it('ignores duplicate items', () => {
      ...
      expect(fileReport.uniqueIssues).to.equal(1);
      expect(fileReport.contentArray).to.have.length(40);
      expect(fileReport.itemsByLine).to.deep.equal({'6' : [ items[0] ]});
    });
    ...
    it('allows multiple files to be added', () => {
      ...
      expect(file1Report.itemsByLine).to.deep.equal({'5' : file1Items});
      ...
      expect(file2Report.itemsByLine)
          .to.deep.equal({'5' : [ file2Items[0] ], '6' : [ file2Items[1] ]});
    });
  });
