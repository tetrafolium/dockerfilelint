...
module.exports.run =
    function(configPath, content) {
  // Parse the file into an array of instructions
  var instructions = {};
  var lines = content.split(/\r?\n/) || [];

  // Supporting multi-line commands, read each line into an array of
  // "instructions", noting the original line number the instruction started on
  var partialLine = '';
  ...
  lines.forEach(function(line, lineNum) {
    ...
    // We want the errors to report the correct line number, so get the start of
    // the extended, multi-line instruction
    if (partialLineNum !== 0) {
  ...
  for (var idx in instructions) {
    ...
    if (result.command === 'cmd') {
      ...
      state.stages.push({instructionsProcessed : 0, cmdFound : false});
    }
...
function loadRules(configPath) {
  ...
  var configFileContents =
      tryLoadFile(path.join(configPath, '.dockerfilelintrc'))
...
function runLine(state, instructions, idx) {
  // return items in an object with the line number as key, value is array of
  // items for this line
  var items = [];
  ...
  // check that the first command is a FROM, this might get reported twice, if
  // the FROM command does exist, but is not the time (non blank, non commented)
  // line
  if (state.stages[state.stages.length - 1].instructionsProcessed === 0 &&
      (cmd !== 'arg' && cmd !== 'from')) {
  ...
  if (args) {
    ...
    switch (cmd) {
    case 'from':
      checks.base_image_tag(args).forEach(function(
          item) { items.push(messages.build(state.rules, item, line)); });
      break;
    case 'maintainer':
      checks.valid_maintainer(args).forEach(function(
          item) { items.push(messages.build(state.rules, item, line)); });
      break;
    case 'run':
      ...
      if (aptgetSubcommands.indexOf('update') > -1 &&
          aptgetSubcommands.indexOf('install') === -1) {
        items.push(messages.build(state.rules, 'apt-get-update_require_install',
                                  line));
      }
      ...
      apk.apk_commands(commands).forEach(function(apk_command, index) {
        var subcommand = apk.apk_subcommand(apk_command);
        if (subcommand === 'add') {
          // If there is more than 1 package we are adding and we are also
          // del'ing, we should suggest --virtual
          if (apk.apkadd_numpackages(apk_command) > 1) {
            if (num_apkdel > 0) {
              if (!apk.apkadd_hasvirtual(apk_command)) {
                items.push(messages.build(state.rules, 'apkadd-missing-virtual',
                                          line));
              }
            }
          }
          if (!apk.apkadd_hasnocache(apk_command)) {
            if (!apk.apkadd_hasupdate(apk_command) ||
                !apk.follows_rmapkcache(commands, index)) {
              items.push(messages.build(
                  state.rules, 'apkadd-missing_nocache_or_updaterm', line));
            }
          }
        }
      });
    ...
    case 'cmd':
      break;
    case 'label':
      checks.label_format(args).forEach(function(
          item) { items.push(messages.build(state.rules, item, line)); });
      break;
    case 'expose':
      checks.expose_container_port_only(args).forEach(function(
          item) { items.push(messages.build(state.rules, item, line)); });
      ...
      break;
    case 'env':
      checks.is_valid_env(args).forEach(function(
          item) { items.push(messages.build(state.rules, item, line)); });
      break;
    case 'add':
      checks.is_valid_add(args).forEach(function(
          item) { items.push(messages.build(state.rules, item, line)); });
      break;
    case 'copy':
      break;
    ...
    case 'volume':
      break;
    case 'user':
      checks.valid_user(args).forEach(function(
          item) { items.push(messages.build(state.rules, item, line)); });
      break;
    case 'workdir':
      checks.is_valid_workdir(args).forEach(function(
          item) { items.push(messages.build(state.rules, item, line)); });
      break;
    case 'arg':
      break;
    ...
    case 'stopsignal':
      break;
    case 'shell':
      checks.is_valid_shell(args).forEach(function(
          item) { items.push(messages.build(state.rules, item, line)); });
      break;
    default:
  ...
  return {command : cmd, items : items};
}
