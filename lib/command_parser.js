...
var command_parser = module.exports = {
  ...
  split_commands : function(args) {
    ...
    var c = commands.map(function(command) { return command.join(' '); });
    return c;
  },
  ...
  command_has_part : function(command, commandPart) {
    return command.match(/\S+/g).some(function(
        part) { return part.trim() === commandPart; });
  },

  command_has_flag : function(command, commandFlag) {
    if (commandFlag.startsWith('--')) {
      return command.match(/\S+/g).some(function(
          part) { return part.trim() === commandFlag; });
    } else if (commandFlag.startsWith('-')) {
      return command.match(/\S+/g).some(function(part) {
        if (part.startsWith('--') || !part.startsWith('-')) {
          return false;
        }
        return part.trim().substr(1).split('').some(function(
            char) { return char === commandFlag.substr(1); });
      });
    }
    throw new Error('bad flag');
  },

  get_nextcommands : function(commands,
                              index) { return commands.slice(index + 1); }
};
