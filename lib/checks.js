...
var commands = module.exports = {
  ...
  expose_port_valid : function(port) {
    return /^([0-9]{1,4}|[1-5][0-9]{4}|6[0-4][0-9]{3}|65[0-4][0-9]{2}|655[0-2][0-9]|6553[0-5])(\/tcp|\/udp){0,1}$/
        .test(port);
  },
  ...
  base_image_tag : function(args) {
    // scratch is a special base layer for Docker, it means there is no base
    // layer
    if (args === 'scratch') {
      return [];
    }
  ...
  param_count_min : function(
      args, min_count) { return args.match(/\S+/g).length >= min_count; },

  is_dir_in_context : function(
      dir) { return (!dir.startsWith('..') && !dir.startsWith('/')); },
  ...
  is_valid_add : function(args) {
    ...
    // if there are > 1 source or any source contains a wildcard, then dest must
    // be a dir
