...
class CliReporter extends Reporter {
  constructor(opts) {
    ...
    this.ui = cliui(opts);
    this.issueTitleWidth = Math.min(
        ISSUE_TITLE_WIDTH_MAX,
        parseInt((opts.width - ISSUE_COL0_WIDTH - ISSUE_COL1_WIDTH - 2) / 3.95,
                 10));
    this.styles = {
  ...
  buildReport() {
    ...
    Object.keys(self.fileReports).forEach((file) => {
      ...
      self.ui.div({text : 'File:   ' + file, padding : PAD_TOP1_LEFT0});
      ...
      linesWithItems.forEach((lineNum) => {
        self.ui.div({
          text : 'Line ' + lineNum + ': ' +
                     chalk.magenta(
                         fileReport.contentArray[parseInt(lineNum, 10) - 1]),
          padding : PAD_TOP1_LEFT0
        });
        self.ui.div({text : 'Issue', width : ISSUE_COL0_WIDTH}, {
          text : 'Category',
          padding : PAD_TOP0_LEFT2,
          width : ISSUE_COL1_WIDTH
        },
                    {
                      text : 'Title',
                      padding : PAD_TOP0_LEFT2,
                      width : self.issueTitleWidth
                    },
        ...
        fileReport.itemsByLine[lineNum].forEach((item) => {
          ...
          self.ui.div(
              {
                text : style(String(itemNum++)),
                width : ISSUE_COL0_WIDTH,
                align : 'right'
              },
              {
                text : style.inverse(item.category),
                padding : PAD_TOP0_LEFT2,
                width : ISSUE_COL1_WIDTH
              },
              {
    ...
    self.ui.div();
    return {
      toString : self.ui.toString.bind(self.ui),
