...
var apt = module.exports = {
  aptget_commands : function(
      commands) { return command_parser.filter_commands(commands, 'apt-get'); },

  // return the subcommand from apt-get
  aptget_subcommand : function(
      aptgetCommand) { return command_parser.get_subcommand(aptgetCommand); },
  ...
  aptget_hasnorecommends : function(aptgetCommand) {
    return command_parser.command_has_flag(aptgetCommand,
                                           '--no-install-recommends');
  },

  // check that command includes a rm -rf /var/lib/apt/lists/*
  follows_rmaptlists : function(commands, index) {
    return command_parser.get_nextcommands(commands, index)
        .some(function(command) {
          if (command.split(' ')[0] !== 'rm') {
            return false;
          }
          if (!command_parser.command_has_part(command,
                                               '/var/lib/apt/lists/*')
          ) {
            return false;
          }
