...
* * *
#### \`RUN\` continuation
If a \`RUN\` line ends with a \`\\\`, the next line will be treated as part of the same \`RUN\` argument.
* * *
#### Blank or Whitespace
...
* * *`,
  ...
  'uppercase_commands' : {
    ...
    'description' :
        `For clarity and readability, all instructions in a Dockerfile should be uppercase.
...
and you should be able to easily see the instructions used.`,
  ...
  'from_first' : {
    ...
    'description' :
        'The first instruction in a Dockerfile must specify the base image using a FROM command or an ARG command to modify the FROM command.  Additionally, FROM cannot appear later in a Dockerfile.',
    'category' : 'Possible Bug'
  },
...
more information, see [this blog post](http://blog.replicated.com/2016/02/05/refactoring-a-dockerfile-for-image-size/)`,
  ...
  'apt-get-upgrade' : {
    ...
    'description' :
        `Use of \`apt-get upgrade\` is not allowed in a Dockerfile.  From the [best practices - take 2](http://crosbymichael.com/dockerfile-best-practices-take-2.html) blog post:
...
> If there are security updates that a base image needs, let upstream know so that they can update it for everyone and ensure that your builds are consistent again.`,
  ...
  'apkadd-missing_nocache_or_updaterm' : {
    ...
    'description' :
        `Consider using a \`--no-cache\` (supported in alpine linux >= 3.3) or \`--update\` followed by the command \`rm -rf /var/cache/apk/*\` when \`apk\` adding packages.  This will result in a smaller image size.  For
more information, see [this link](https://github.com/gliderlabs/docker-alpine/blob/master/docs/usage.md)`,
  ...
  'apkadd-missing-virtual' : {
    ...
    'description' :
        `Consider using a \`--virtual\` or \`-t\` switch to group multiple packages for easy cleanup.  This will help ensure future authors will continue to clean up build dependencies and other temporary packages.  For
more information, see [this link](https://github.com/gliderlabs/docker-alpine/blob/master/docs/usage.md#virtual-packages)`,
  ...
  'expose_host_port' : {
    ...
    'description' :
        `Using \`EXPOSE\` to specify a host port is not allowed.  While early versions of Docker supported this syntax, it has been deprecated from some time now.
